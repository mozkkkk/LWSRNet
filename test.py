...
def test(test_loader, Eva_test, save_path, net):
    ...
    for i, (A, B, mask, filename) in enumerate(tqdm(test_loader)):
        with torch.no_grad():
            ...
            Y = mask.cuda()
            preds = net(A,B)
            output1 = F.sigmoid(preds[1])
            heatmap = output1.clone()
            ...
            for i in range(output.shape[0]):
                ...
                heat = (torch.squeeze(heatmap[i])).data.cpu().numpy()
                #print(heat.shape)
                final_mask = probs_array * 255
                final_mask = final_mask.astype(np.uint8)
                final_savepath = save_path + filename[i] + '.png'
                heat_savepath = save_path + filename[i] + '.npy'
                im = Image.fromarray(final_mask)
                #np.save(heat_savepath,heat)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_name', type=str, default='DSIFN', #修改这里！！！
                        help='the test rgb images root')
    parser.add_argument('--model_name', type=str, default='LwSRNet', #修改这里！！！
                        help='the test rgb images root')
