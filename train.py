...
from network.LwSRNet import LwSRNet
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_name', type=str, default='S2Looking', #修改这里！！！
    ...
    if opt.data_name == 'LEVIR':
        ...
        opt.val_root = '../LEVIR/test/'
    elif opt.data_name == 'WHU':
        opt.train_root = '../WHU/train/'
        opt.val_root = '../WHU/test/'
    elif opt.data_name == 'CDD':
        opt.train_root = '../CDD/train/'
        opt.val_root = '../CDD/test/'
    elif opt.data_name == 'DSIFN':
        opt.train_root = '../DSIFN/train/'
        opt.val_root = '../DSIFN/test/'
    elif opt.data_name == 'SYSU':
        opt.train_root = '../SYSU/train/'
        opt.val_root = '../SYSU/test/'
    elif opt.data_name == 'S2Looking':
        opt.train_root = '../S2Looking/train/'
        opt.val_root = '../S2Looking/test/'
    ...
    elif opt.model_name == 'LwSRNet':
        model = LwSRNet().cuda()

    n_parameters = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print('number of params:', n_parameters)
    test_input1 = torch.randn(1, 3, 128, 128).float().cuda()  # 输入尺寸需匹配模型
    test_input2 = torch.randn(1, 3, 128, 128).float().cuda()  # 输入尺寸需匹配模型
    ...
    lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=opt.epoch, T_mult=2)
